...
import (
	...
	"github.com/stevemk14ebr/GoReSym/debug/pe"
)
...
func (f *peFile) pcln_scan() (secStart uint64, pclntabRVA uint64, pclntab []byte, err error) {
	var imageBase uint64
	switch oh := f.pe.OptionalHeader.(type) {
	case *pe.OptionalHeader32:
		imageBase = uint64(oh.ImageBase)
...
func (f *peFile) pcln() (secStart uint64, symtab []byte, pclntabVA uint64, pclntab []byte, err error) {
